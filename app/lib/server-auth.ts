...
export async function getCurrentUser(): Promise<SteamUser | null> {
  try {
    ...
    // the framework's signal that the route is dynamic. Re-throw so Next
    // can catch it, but don't pollute the logs with a bogus entry.
...
/** Returns the authenticated admin user or throws if not admin. */
export async function requireAdmin(): Promise<SteamUser> {
